...
Doctype = 'Peer reviewed publications'
pattern_for_wtg = r'\((\s*(?:\d+\.\d+|\d+)\s*)\)'

class Peerreviewedpublications(Document):
	
	"""method to autoname your document"""
	def autoname(self):
		self.name = f'RB7_{self.professor}_{self.academic_year}_{self.semester}'
...
def compute_marks(self):
	...
	for item in self.criteria_table:
		if counter > 1:
			...
			marks_obtained = round(val1*val2*100 , 2)
