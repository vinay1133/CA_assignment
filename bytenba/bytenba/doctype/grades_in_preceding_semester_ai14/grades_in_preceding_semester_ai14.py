...
doctype = 'Grades in preceding semester AI14'

class GradesinprecedingsemesterAI14(Document):
	def before_save(self):
		if self.precd_sem_grade=="A":
			self.self_appraisal_score = 150
		elif self.precd_sem_grade=="B":
			self.self_appraisal_score = 75
		elif self.precd_sem_grade=="C":
			self.self_appraisal_score = 40
		else:
			self.self_appraisal_score = 0

	"""method to autoname your document"""
	def autoname(self):
		base_name = f'AI14_{self.academic_year}_{self.professor}'
...
def renameDoc(base_name, academic_year):
	...
	field = 'name'
	similar_docs = frappe.get_list("Grades in preceding sem preview AI14", filters= filters, pluck = field)
