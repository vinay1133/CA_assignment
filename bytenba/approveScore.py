...
def approve(**kwargs):
  ...
  if is_admin:
    ...
    if not is_reviewer:
      frappe.throw("""From cannot be validated by faculty""")

    #check if user is not approving his own form 
    if user == owner and not is_reviewer:
      frappe.throw("""From cannot be validated by faculty who is the form owner""")

    #check reviewer belongs to same department        
    form_owner_dept = frappe.db.get_value("Professors", owner, "department")
    dept_hod = frappe.db.get_value('Professors', {"is_hod":1,"department": form_owner_dept}, 'name')
    if dept_hod != user:
...
def revoke(**kwargs):
  ...
  if is_admin:
    ...
    #check if user is not approving his own form 
    if user == owner and not is_reviewer:
      frappe.throw("""From cannot be revoked by faculty who is the form owner""")

    #check reviewer belongs to same department        
    form_owner_dept = frappe.db.get_value("Professors", owner, "department")
    dept_hod = frappe.db.get_value('Professors', {"is_hod":1,"department": form_owner_dept}, 'name')
    if dept_hod != user:
      frappe.throw("""From can only be revoked by reviewer belonging to same department as form owner""")
